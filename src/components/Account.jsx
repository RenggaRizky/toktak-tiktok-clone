...
const Account = ({ url, name, img, height, width, nameStyle, usernameStyle, hide = true }) => {
    return (
        <>
            <Link href={url} className={[hide ? "d-lg-flex align-items-center" : "d-flex align-items-start align-items-lg-center", "rouded-circle mb-3"].join(" ")}>
                <Image src={img} height={height} width={width} alt={name} className={[hide ? "" : "me-3 me-lg-0", ""].join(" ")} />
                <div className={[hide ? "d-none d-lg-inline-block " : "d-lg-flex align-items-lg-center", "ms-lg-3"].join(" ")}>
                    ...
                    <p className={[hide ? `` : `ms-lg-2`, `${usernameStyle} mb-0 text-gray`].join(" ")}>{name}</p>
                </div>
            </Link>
        </>
    );
